...
class SrsRepositoryImpl(
    ...
    override suspend fun getCard(wordId: Int): SrsCard? = withContext(Dispatchers.Default) {
        val cursor = driver.executeQuery(
            ...
            mapper = { cursor ->
                if (cursor.next().value) cursor.toSrsCard() else null
            },
            parameters = 1
        ) {
    ...
    override suspend fun getNewCards(limit: Int): List<Int> = withContext(Dispatchers.Default) {
        // Return word IDs that don't have an SRS card yet (never studied)
        val cursor = driver.executeQuery(
            identifier = null,
...
private fun app.cash.sqldelight.db.SqlCursor.toSrsCard() = SrsCard(
