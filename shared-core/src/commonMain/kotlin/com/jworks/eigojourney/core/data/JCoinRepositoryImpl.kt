...
import com.jworks.eigojourney.core.domain.model.CoinTier
import com.jworks.eigojourney.core.domain.model.PurchaseResult
import com.jworks.eigojourney.core.domain.model.ShopCategory
...
class JCoinRepositoryImpl(
    private val database: EigoJourneyDatabase,
    ...
    private fun buildAppKeyHeaders(): Headers = Headers.build {
    ...
    override suspend fun syncPendingEvents(): Int = withContext(Dispatchers.Default) {
        if (!SupabaseClientFactory.isInitialized() || appKey.isBlank()) {
        ...
        for (event in pending) {
            try {
                val functionName = when (event.event_type) {
                    "spend" -> "jcoin-unified-spend"
                    else -> "jcoin-unified-earn"
                }
                ...
                val body = buildJsonObject {
                    ...
                    if (event.event_type == "spend") {
                        ...
                        put("trigger_type", event.source_type)
                    }
                }
...
private fun com.jworks.eigojourney.db.Coin_balance.toDomain() = CoinBalance(
    userId = user_id,
